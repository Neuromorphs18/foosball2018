...
class Agent0(object):
    ...
    def predict_player_intersections(self, ball_pos, ball_vel, ref_time, rec_depth=0):
        ...
        ball_line = self.line([ball_pos[0], ball_pos[1]],
                              [ball_pos[0] + ball_vel[0] * 1.0, ball_pos[1] + ball_vel[1] * 1.0])
        ...
        for idx, player_line in enumerate(self.player_lines):
            if (ball_vel[0] > 0 and self.players[idx].x > ball_pos[0]) or (
                    ball_vel[0] < 0 and self.players[idx].x < ball_pos[0]):
        ...
        if not all([i is None for i in player_intersections]):
            ...
            wall0 = self.right_wall if ball_vel[
                                           0] > 0 else self.left_wall  # consider ball vector direction when evaluating walls.
            ...
            wall_intersections = [intersection for intersection in wall_intersections if intersection not in [None , False]]
            wall_collisions = [(x, y) for (x, y) in wall_intersections if
                               x >= self.left_bound and x <= self.right_bound and y >= self.upper_bound and y <= self.lower_bound]
    ...
    def servo_players(self):
        ...
        for idx, target_y in enumerate(self.target_positions):
            if target_y is not None:
                ...
                candidate_ys = [idx_ for idx_, y in enumerate(self.players[idx].ys) if
                                target_y >= y and target_y <= y + self.players[idx].max_y]
    ...
    def select_action(self):
        ...
        player_intersections = self.predict_player_intersections(self.env.ball_pos, self.env.ball_vel,
                                                                 0.01)  # Note: Hardcoded time.
        for idx, intersection in enumerate(player_intersections):
            if intersection is not None:
                ...
                if moving_right == self.goal_left:  # Slightly confusing.
                    ...
                    self.target_positions[idx] = intersection[0] + 200.0 / len(
                        self.players[idx].ys)  # move intersecting players out of the way.
        if self.randomness:
            self.target_positions = [None if pos is None else (pos + (random.random() - 0.5) * 20.0) for pos in
